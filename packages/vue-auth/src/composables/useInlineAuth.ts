...
export const useInlineAuth = <AU = AuthUser> () => {
     ...
     * @returns 
     */
    const useActionWithCallbacks = <R extends { error?: BaseError }> (
        action: () => Promise<R>
    ) => {
    ...
    const login = <U = AU, T = LoginCredentials> (
        ...
        const action = async (): Promise<UnrefData<AuthData<U>>> => {
            ...
            const res: UnrefData<AuthData<U>> = { user: data.user, message: data.message, token: data.token }
            if (data.error) {
    ...
    const forgot = <T = unknown> (
        ...
        const action = async (): Promise<UnrefData<ForgotData>> => {
            ...
            return { error: data.error, message: data.message, timeout: data.timeout, countdown: countdown.value }
        }
